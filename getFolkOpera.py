...
class GetFanJuData(object):
    def __init__(self):
        self.basePreUrl = 'https://api.bilibili.com/pgc/season/index/result?season_version=-1&area=-1&is_finish=-1&copyright=-1&season_status=-1&season_month=-1&year=-1&style_id=-1&order=3&st=1&sort=0&page='
    ...
    def save_data(self):
        ...
        """
        try:
            sql = 'insert into media values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) on duplicate key update badge=values(badge),cover=values(cover),index_show=values(index_show),link=values(link),folk_opera_order=values(folk_opera_order),order_type=values(order_type),season_id=values(season_id),is_finish=values(is_finish),folk_oprea_comment=values(folk_oprea_comment)'
